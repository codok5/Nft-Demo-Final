    ...
    KIP17Pausable
{
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(AccessControlEnumerable,KIP17 ,KIP17Burnable, KIP17Mintable, KIP17Pausable, KIP17Enumerable)
        returns (bool)
    {
        return
            AccessControlEnumerable.supportsInterface(interfaceId) ||
    ...
    uint256 private _limitperblock;



    function publicmintenable(bool enable) public onlyRole(MINTER_ROLE){
        _pubmintenable = enable;
    }
    ...
    function setupminting(uint256 startblocknumber, uint256 maxamount, uint256 price, 
                        uint256 limitperblock, uint256 newcurrentindex) external onlyRole(MINTER_ROLE) {
        _startblocknumber = startblocknumber;
        _maxamount = maxamount;
        _price = price;
        _limitperblock = limitperblock;
        _currentindex = newcurrentindex;
    }

    
    function publicMint(uint256 requestedCount) external payable{
        require(_pubmintenable, "Public Minting is not enabled");
        require(block.number >= _startblocknumber, "Wait a minute please.");
        require(requestedCount >0  && requestedCount <= _limitperblock,"Not proper request");
        require(msg.value == SafeMath.mul(_price,requestedCount), "Not enough Klay");
        require(SafeMath.add(_currentindex,requestedCount) <= _maxamount + 1, "Exceed max amount");
        for(uint256 i=0; i<requestedCount; i++){
            _mint(msg.sender,_currentindex);
            _setTokenURI(_currentindex,bytes(_baseuri).length > 0 ? string(abi.encodePacked(_baseuri, Strings.toString(_currentindex),".json")) : "");
            _currentindex = SafeMath.add(_currentindex,1);
        }
    }

    function mintingInformation() external view returns (uint256[5] memory){
      uint256[5] memory info =
        [_currentindex, _limitperblock, _startblocknumber, _maxamount, _price];
      return info;
    }
   

    function withdraw(address payable _to) external onlyRole(MINTER_ROLE) {
